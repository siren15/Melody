...
class Tags(Scale):
    ...
    async def t(self, ctx:InteractionContext, tagname:str):
        await ctx.defer()
        is_ext_active = await is_extension_active(ctx)
        if is_ext_active == True:
    ...
    async def tag_list(self, ctx:InteractionContext):
        ...
        if is_ext_active == True:
            ...
            if hasrole == True:
                ...
                embedl = await ctx.send(embed=newpage(create_list(names, 20, s, e), ctx.guild, footer), components=reaction_buttons)
